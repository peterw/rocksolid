...
```bash
mkvirtualenv upwork_test -p python3.11
...
Create a database named `upwork_test`.

```
createdb upwork_test
```
...
```bash
celery -A upwork_test worker -l INFO --pool=solo
```

Or with celery beat (for scheduled tasks):

```bash
celery -A upwork_test worker -l INFO -B --pool=solo
```
